...
class Main extends Component {
  ...
  loadNotebooks = async (userData, userToken) => {
    ...
    this.setState({ notebooks: [...notebooks] }, () => notebooks.length > 0 ? this.loadNotes(0) : this.createDefaultNotebook())
  ...
  loadNotes = async (notebookIndex) => {
    ...
    this.setState({
      ...
      currentNotebookIndex: notebookIndex, isEditable: false
    })
  }
        ...
        <Container fluid >
          <Row>
            ...
            <Col
              ...
              tag="main"
            >
              <TopBar userData={userData} token={token} handleSearchChange={this.handleSearchNotes} searchValue={searchTerm} profileImage={profileImage} key={userData && userData._id && profileImage} />
              <Switch>
                <Route exact path={match.path} render={() => (
                  <Notes userData={userData && userData} loading={loading}
